...
class ModuleVisibilityTests(unittest.TestCase):
    """The visibility sentence names modules by contract, so both halves count."""

    def setUp(self) -> None:
        self.modules = declared_modules()
        self.checked_out = {
            name: ROOT / "modules" / name / ".gds" / "repository.yaml"
            for name in self.modules
        }
        if not all(path.is_file() for path in self.checked_out.values()):
            self.skipTest("not every module is checked out; visibility is unreadable")

    def visibility(self, name: str) -> str:
        anchor = yaml.safe_load(self.checked_out[name].read_text(encoding="utf-8"))
        return anchor["classification"]["visibility_contract"]

    def test_the_module_count_matches_the_typed_relationships(self) -> None:
        expected = WORDS[len(self.modules)].lower()
        self.assertIn(f"its {expected} modules", overview())

    def test_every_public_module_is_named_as_public(self) -> None:
        text = overview()
        section = text[text.index("**Repository visibility**") :]
        section = section[: section.index("\n- ")]
        for name in sorted(self.modules):
            if self.visibility(name) == "public":
                self.assertIn(
                    f"`{name}`", section, f"{name} is public and is not named as such"
                )

    def test_no_private_module_is_listed_among_the_public_ones(self) -> None:
        text = overview()
        section = text[text.index("**Repository visibility**") :]
        section = section[: section.index("\n- ")]
        public_clause = section[: section.index("carry `visibility_contract: public`")]
        for name in sorted(self.modules):
            if self.visibility(name) != "public":
                self.assertNotIn(
                    f"`{name}`",
                    public_clause,
                    f"{name} is not public but appears in the public clause",
                )
