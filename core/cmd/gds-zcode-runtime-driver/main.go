...
func main() {
	if len(os.Args) != 1 {
		fmt.Fprintln(os.Stderr, "gds-zcode-runtime-driver accepts one JSON request on stdin and no arguments")
		os.Exit(64)
	}
	...
	defer cancel()
	evidence, err := harness.RunZcodeEvidenceDriver(
		ctx, request, schemas, harness.ZcodeEvidenceDriverOptions{Concurrency: 2},
	)
	if err != nil {
		fmt.Fprintln(os.Stderr, "zcode runtime evidence failed:", err)
		os.Exit(2)
	}
	encoded, err := json.MarshalIndent(evidence, "", "  ")
	if err != nil {
		fmt.Fprintln(os.Stderr, "encode zcode runtime evidence:", err)
		os.Exit(14)
	}
	encoded = append(encoded, '\n')
	if _, err := io.Copy(os.Stdout, bytes.NewReader(encoded)); err != nil {
		fmt.Fprintln(os.Stderr, "write zcode runtime evidence:", err)
		os.Exit(14)
	}
}
