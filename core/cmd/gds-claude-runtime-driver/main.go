...
func main() {
	if len(os.Args) != 1 {
		fmt.Fprintln(os.Stderr, "gds-claude-runtime-driver accepts one JSON request on stdin and no arguments")
		os.Exit(64)
	}
	...
	defer cancel()
	evidence, err := harness.RunClaudeEvidenceDriver(
		ctx, request, schemas, harness.ClaudeEvidenceDriverOptions{Concurrency: 2},
	)
	if err != nil {
		fmt.Fprintln(os.Stderr, "Claude runtime evidence failed:", err)
		os.Exit(2)
	}
	encoded, err := json.MarshalIndent(evidence, "", "  ")
	if err != nil {
		fmt.Fprintln(os.Stderr, "encode Claude runtime evidence:", err)
		os.Exit(14)
	}
	encoded = append(encoded, '\n')
	if _, err := io.Copy(os.Stdout, bytes.NewReader(encoded)); err != nil {
		fmt.Fprintln(os.Stderr, "write Claude runtime evidence:", err)
		os.Exit(14)
	}
}
