...
- **Three devices**: `example-user-mac1`, `example-user-mac2` (macos/arm64,
  desktop), and `example-user-ubuntu-1` (linux/x86_64, desktop-builds, rootful
  Docker).
- **Repository visibility**: private (ADR 0033). The control-plane repo is
  private; of its four modules, `ci-workflows`, `macos-ubuntu-bootstrap` and
  `agent-runtime` carry `visibility_contract: public`, and `github-actions`
  is private.
