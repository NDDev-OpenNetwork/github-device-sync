...
```bash
GOTOOLCHAIN=go1.27.1 go build -trimpath -o /tmp/gds-controller ./core/cmd/gds-controller
/tmp/gds-controller --version
python3 scripts/validate_gds_schemas.py --root . --json
GOTOOLCHAIN=go1.27.1 go test ./core/controller ./core/webhooks ./core/state -race -count=1
```
