...
`blocking: false` and `delivery_state: unassigned`.
...
always-on GitHub inbox scheduler. The repository owner assigns the issue to an
agent through the existing work process. This caller does not require automatic
agent startup, model selection or a separate supervisor. Do not assume an
...
Actions workflow. Monitor feedback delivery failures as well as check failures;
missed-event reconciliation is separate adoption work. Cancelled attempts reach
the publisher so already-failed jobs remain visible; clean cancellation creates
no issue. Deduplication trusts the configured immutable publisher identity, not
any bot account or arbitrary marker in another user's issue.
...
one real failed attempt, exact repository-local issue publication and duplicate
redelivery behavior. Agent assignment remains the owner's choice. No synthetic production
...
absence of checkout/secret inheritance. They do not prove live delivery,
repository-wide policy migration or live performance.
